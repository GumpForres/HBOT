...
echo
echo
echo "===============  CREATE A NEW HBOT INSTANCE ==============="
echo
echo
...
read -p "   Enter Hbot version you want to use [latest/development] (default = \"latest\") >>> " TAG
if [ "$TAG" == "" ]
then
  TAG="latest"
fi
...
read -p "   Enter a name for your new Hbot instance (default = \"hbot\") >>> " INSTANCE_NAME
if [ "$INSTANCE_NAME" == "" ]
then
  INSTANCE_NAME="hbot"
...
read -p "   Enter a folder name where your Hbot files will be saved (default = \"$DEFAULT_FOLDER\") >>> " FOLDER
if [ "$FOLDER" == "" ]
then
...
create_instance () {
 echo
 echo "Creating Hbot instance ... Admin password may be required to set the required permissions ..."
 echo
 # 1) Create main folder for your new instance
 mkdir $FOLDER
 # 2) Create subfolders for hbot files
 mkdir $CONF_FOLDER
 ...
 # 3) Set required permissions to save hbot password the first time
 sudo chmod a+rw $CONF_FOLDER $CERTS_FOLDER
 # 4) Launch a new instance of hbot
